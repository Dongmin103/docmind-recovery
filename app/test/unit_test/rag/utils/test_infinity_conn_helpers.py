...
def test_dense_only_filter_keeps_active_chunk_scope_condition():
    active_filter = "chunk_set_id IN ('set-active')"
    assert rag_infinity_conn._build_dense_filter(active_filter, "", 0.95) == active_filter


def test_hybrid_dense_filter_keeps_text_and_active_scope_when_text_weight_dominates():
    active_filter = "chunk_set_id IN ('set-active')"
    fulltext_filter = f"({active_filter}) AND filter_fulltext('content_ltks', 'validation')"
    assert rag_infinity_conn._build_dense_filter(active_filter, fulltext_filter, 0.3) == fulltext_filter
