...
def _load_delete_datasets_module(monkeypatch, *, f2d_rows, file_filter_delete, stranded_doc_rows=0):
    ...
    class StubDocmindProtectedEvidenceError(PermissionError):
        code = "DOCMIND_VERSION_PROTECTED"

    _stub(
        monkeypatch,
        "api.db.services.document_service",
        DocumentService=SimpleNamespace(
            assert_documents_docmind_evidence_mutable=lambda *_args, **_kwargs: None,
            query=lambda kb_id: [doc],
            remove_document=lambda doc, tenant_id: True,
            filter_delete=cleanup.doc_filter_delete,
        ),
        DocmindProtectedEvidenceError=StubDocmindProtectedEvidenceError,
