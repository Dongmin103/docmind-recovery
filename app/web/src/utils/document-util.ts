...
export const isValidChunkPosition = (value: unknown): value is number[] =>
  Array.isArray(value) &&
  value.length >= 5 &&
  value.slice(0, 5).every((coordinate) => Number.isFinite(coordinate));

export const getValidChunkPositions = (positions: unknown): number[][] =>
  Array.isArray(positions) ? positions.filter(isValidChunkPosition) : [];

export const buildChunkHighlights = (
  selectedChunk: IChunk | IReferenceChunk,
  size: { width: number; height: number },
) => {
  return getValidChunkPositions(selectedChunk?.positions).map((x) => {
    const boundingRect = {
      width: size.width,
      height: size.height,
      x1: x[1],
