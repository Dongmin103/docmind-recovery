...
func (s *RetrievalService) Retrieval(ctx context.Context, req *RetrievalRequest) (*RetrievalResult, error) {
	...
	candidateWindow := *req.RerankCandidatesCount
	if req.RerankCandidateLimit != nil {
		candidateWindow = *req.RerankCandidateLimit
	}
	if candidateWindow <= 0 || req.Page*pageSize > candidateWindow {
		return nil, fmt.Errorf("rerank candidate window(%d) must be greater than or equal to page(%d) * page_size(%d)", candidateWindow, req.Page, pageSize)
	}
	common.Debug("Retrieval rerank candidate params", zap.Int("page", req.Page), zap.Int("pageSize", pageSize), zap.Int("candidateWindow", candidateWindow))
