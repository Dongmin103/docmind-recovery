...
func validateSearchDatasetsRequest(req *service.SearchDatasetsRequest) error {
	if err := validateSearchParams(req.Page, req.Size, req.KNNTopK, req.TopK, req.KNNNumCandidates, req.SimilarityThreshold, req.VectorSimilarityWeight); err != nil {
		return err
	}
	return validateRerankCandidateLimit(req.RerankCandidateLimit)
}
...
func validateRerankCandidateLimit(limit *int) error {
	if limit != nil && (*limit < 1 || *limit > 500) {
		return fmt.Errorf("rerank_candidate_limit must be between 1 and 500")
	}
	return nil
}
