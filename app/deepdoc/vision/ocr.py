...
from .paddleocr_recognizer import PaddleOCRKoreanV5Recognizer
...
class OCR:
    def __init__(self, model_dir=None):
        ...
        """
        text_recognizer = os.environ.get("DEEPDOC_TEXT_RECOGNIZER", "deepdoc-ctc").strip().lower()
        if text_recognizer not in {"deepdoc-ctc", "paddleocr-korean-v5"}:
            raise ValueError(
                "DEEPDOC_TEXT_RECOGNIZER must be one of: deepdoc-ctc, paddleocr-korean-v5"
            )

        def create_text_recognizer(device_id: int | None = None):
            if text_recognizer == "paddleocr-korean-v5":
                return PaddleOCRKoreanV5Recognizer(device_id)
            return TextRecognizer(model_dir, device_id)
        ...
        if settings.PARALLEL_DEVICES > 0:
            ...
            for device_id in range(settings.PARALLEL_DEVICES):
                ...
                self.text_recognizer.append(create_text_recognizer(device_id))
        else:
            self.text_detector = [TextDetector(model_dir)]
            self.text_recognizer = [create_text_recognizer()]
