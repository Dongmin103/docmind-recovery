...
class RAGFlowPdfParser:
    ...
    def _assign_column(self, boxes, zoomin=3):
        ...
        for pg, bxs in by_page.items():
            ...
            distinct_x0 = len(np.unique(x0s))
            max_try = min(4, len(bxs), distinct_x0)
            if max_try < 2:
                max_try = 1
            best_k = 1
            best_score = -1
            ...
            for k in range(1, max_try + 1):
                ...
                if len(centers) > 1:
                    minimum_cluster_size = max(2, math.ceil(len(bxs) * 0.1))
                    counts = np.bincount(labels, minlength=k)
                    if np.min(counts) < minimum_cluster_size:
                        continue
                    if np.min(np.diff(centers)) < width * 0.28:
                        continue
                    try:
    ...
    def _final_reading_order_merge(self, zoomin=3):
        ...
        for pg in sorted(pages.keys()):
            page_boxes = pages[pg]
            page_left = min(box["x0"] for box in page_boxes)
            page_right = max(box["x1"] for box in page_boxes)
            page_width = max(1.0, page_right - page_left)
            median_height = float(
                np.median(
                    [
                        max(1.0, box["bottom"] - box["top"])
                        for box in page_boxes
                    ]
                )
            )
            gap_threshold = max(2.0, median_height * 1.5)

            vertical_bands = []
            current_band = []
            current_bottom = None
            for box in sorted(page_boxes, key=lambda item: (item["top"], item["x0"])):
                if (
                    current_band
                    and current_bottom is not None
                    and box["top"] - current_bottom > gap_threshold
                ):
                    vertical_bands.append(current_band)
                    current_band = []
                    current_bottom = None
                current_band.append(box)
                current_bottom = max(current_bottom or box["bottom"], box["bottom"])
            if current_band:
                vertical_bands.append(current_band)

            for band in vertical_bands:
                columns = {box.get("col_id", 0) for box in band}
                if len(columns) <= 1:
                    new_boxes.extend(
                        sorted(band, key=lambda box: (box["top"], box["x0"]))
                    )
                    continue
                ...
                spanning = [
                    box
                    for box in band
                    if (box["x1"] - box["x0"]) / page_width >= 0.65
                ]
                if not spanning:
                    new_boxes.extend(order_band(band))
                    continue

                spanning_ids = {id(box) for box in spanning}
                remaining = [box for box in band if id(box) not in spanning_ids]
                for span in sorted(
                    spanning, key=lambda box: (box["top"], box["x0"])
                ):
                    span_center = (span["top"] + span["bottom"]) / 2
                    before = [
                        box
                        for box in remaining
                        if (box["top"] + box["bottom"]) / 2 < span_center
                    ]
                    if before:
                        new_boxes.extend(order_band(before))
                        before_ids = {id(box) for box in before}
                        remaining = [
                            box for box in remaining if id(box) not in before_ids
                        ]
                    new_boxes.append(span)
                new_boxes.extend(order_band(remaining))
