...
DOCMIND_PROTECTED_EVIDENCE_ERROR_CODE = "DOCMIND_VERSION_PROTECTED"
DOCMIND_PROTECTED_LIFECYCLES = ("READY", "PUBLISHED", "SUPERSEDED")


class DocmindProtectedEvidenceError(PermissionError):
    def __init__(self, operation: str, protected_document_count: int):
        super().__init__(DOCMIND_PROTECTED_EVIDENCE_ERROR_CODE)
        self.code = DOCMIND_PROTECTED_EVIDENCE_ERROR_CODE
        self.operation = operation
        self.protected_document_count = protected_document_count


def _latest_parser_run_tasks(tasks):
    parser_tasks = [task for task in tasks if getattr(task, "parse_run_id", None)]
    if not parser_tasks:
        return tasks
    latest = max(parser_tasks, key=lambda task: getattr(task, "create_time", 0) or 0)
    return [task for task in parser_tasks if task.parse_run_id == latest.parse_run_id]
...
class DocumentService(CommonService):
    ...
    @classmethod
    def assert_docmind_evidence_mutable(cls, document_id: str, operation: str, *, actor_id: str | None = None, trace_id: str | None = None):
        cls.assert_documents_docmind_evidence_mutable(
            [document_id],
            operation,
            actor_id=actor_id,
            trace_id=trace_id,
    ...
    @classmethod
    def assert_documents_docmind_evidence_mutable(
        cls,
        document_ids,
        operation: str,
        *,
        ...
        normalized_ids = sorted({str(document_id) for document_id in document_ids if document_id})
        if not normalized_ids:
            return
        if not DocmindFolderVersionDocument.table_exists() or not DocmindCatalogVersion.table_exists():
            return

        protected_rows = list(
            DocmindFolderVersionDocument.select(
                DocmindCatalogVersion.project_id,
                DocmindFolderVersionDocument.document_id,
            )
            .join(
                DocmindCatalogVersion,
                on=(DocmindFolderVersionDocument.version_id == DocmindCatalogVersion.id),
            )
            .where(
                (DocmindFolderVersionDocument.document_id.in_(normalized_ids))
                & (DocmindCatalogVersion.lifecycle_state.in_(DOCMIND_PROTECTED_LIFECYCLES))
            )
            .distinct()
            .dicts()
        )
        ...
        protected_count = len({row["document_id"] for row in protected_rows})
        project_id = sorted({row["project_id"] for row in protected_rows})[0]
        now = datetime.now()
        ...
        DocmindAuditEvent.create(
            ...
            actor_id=actor_id,
            action="PROTECTED_EVIDENCE_MUTATION_DENIED",
            target_type="DOCUMENT_SET",
            target_id=None,
            before_version_id=None,
            after_version_id=None,
            outcome="DENIED",
            trace_id=trace_id,
            details={"operation": operation, "protected_document_count": protected_count},
            create_time=stamp,
            create_date=now,
            update_time=stamp,
            update_date=now,
        )
        raise DocmindProtectedEvidenceError(operation, protected_count)
    ...
    def remove_document(cls, doc, tenant_id):
        ...
        cls.assert_docmind_evidence_mutable(doc.id, "DELETE_DOCUMENT")
        parser_runtime = ParserPlatformConfig.from_env()
        if parser_runtime.integration_ready and ParserRun.select().where(ParserRun.doc_id == doc.id).exists():
            from api.db.services.chunk_set_activation_service import (
                ACTIVE_SCOPE_CACHE,
                DocStoreChunkSetArtifactCleaner,
                PeeweeAtomicChunkSetStore,
            )
            from rag.parser_platform import ChunkSetActivationCoordinator, ParserArtifactRepository

            artifacts = ParserArtifactRepository(parser_runtime.artifact_root)
            ChunkSetActivationCoordinator(
                PeeweeAtomicChunkSetStore(
                    artifact_cleaner=DocStoreChunkSetArtifactCleaner(
                        raw_artifact_cleaner=artifacts.remove_run_ref,
                        page_artifact_cleaner=artifacts.remove_page_artifacts,
                    )
                ),
                cache=ACTIVE_SCOPE_CACHE,
            ).delete_document_sets(document_id=doc.id)
    ...
    @classmethod
    @DB.connection_context()
    def with_manual_chunk_append_lock(cls, doc_id, operation):
        """Run one manual fallback append while holding the document append lock.

        The operation must derive the fallback ordinal, store/index the chunk,
        and advance counters before the lock is released. This keeps concurrent
        fallback IDs unique without advancing document or KB counts when storage
        or indexing fails.
        """
        lock_name = f"manual_chunk_append:{doc_id}"
        with DB.lock(lock_name):
            return operation()

    @classmethod
    def next_manual_chunk_ordinal(cls, doc_id, kb_id):
        doc = (
            cls.model.select(cls.model.chunk_num)
            .where((cls.model.id == doc_id) & (cls.model.kb_id == kb_id))
            .get()
        )
        return int(getattr(doc, "chunk_num", 0) or 0) + 1
    ...
    def delete_document_and_update_kb_counts(cls, doc_id) -> bool:
        ...
        """
        cls.assert_docmind_evidence_mutable(doc_id, "DELETE_DOCUMENT_ROW")
        with DB.atomic():
            doc = (
                cls.model.select(
                    cls.model.id,
    ...
    def clear_chunk_num(cls, doc_id):
        ...
        cls.assert_docmind_evidence_mutable(doc_id, "CLEAR_DOCUMENT_CHUNK_COUNTS")
    ...
    def clear_chunk_num_when_rerun(cls, doc_id):
        cls.assert_docmind_evidence_mutable(doc_id, "RESET_DOCUMENT_FOR_RERUN")
        doc = cls.model.get_by_id(doc_id)
        assert doc, "Can't find document in database."
    ...
    def update_parser_config(cls, id, config):
        if not config:
            return
        cls.assert_docmind_evidence_mutable(id, "UPDATE_PARSER_CONFIG")
    ...
    def begin2parse(cls, doc_id, keep_progress=False):
        cls.assert_docmind_evidence_mutable(doc_id, "BEGIN_DOCUMENT_PARSE")
        info = {
    ...
    def _sync_progress(cls, docs: list[dict]):
        ...
        for d in docs:
            try:
                ...
                if not tsks:
                    continue
                tsks = _latest_parser_run_tasks(tsks)
                msg = []
                prg = 0
                finished = True
                bad = 0
    ...
    def run(cls, tenant_id: str, doc: dict, kb_table_num_map: dict):
        ...
        # The parser platform is feature-gated and standard-ingestion only. Run
        # this before any mutable guard, task deletion/reuse, queue write, parser
        # service call, or Dataflow conversion so a mode/capability mismatch is
        # fail-closed and leaves the current active evidence untouched.
        enforce_prequeue_gate(doc)

        cls.assert_docmind_evidence_mutable(
            doc["id"],
            "RUN_DOCUMENT_PARSE",
            actor_id=tenant_id,
        )
