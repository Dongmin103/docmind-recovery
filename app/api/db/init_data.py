...
def init_docmind_shared_user():
    """Create the internal shared workspace identity exactly once.

    The generated password is intentionally not printed or persisted outside the
    database because browser login is disabled for this internal-only identity.
    """

    if os.getenv("DOCMIND_SHARED_WORKSPACE_ENABLED", "").strip().lower() not in _ENABLED_VALUES:
        return None

    email = os.getenv("DOCMIND_SHARED_USER_EMAIL", "docmind@internal.invalid").strip().lower()
    if not email:
        raise RuntimeError("DOCMIND_SHARED_USER_EMAIL is required")

    users = UserService.query(email=email)
    if len(users) > 1:
        raise RuntimeError("DOCMIND_SHARED_USER_DUPLICATE")
    if users:
        user = users[0]
        if not str(user.access_token or "").strip():
            UserService.update_by_id(user.id, {"access_token": uuid.uuid1().hex})
        logging.info("DocMind shared workspace identity already exists")
        return user.id
    ...
    result = create_new_user(
        {
            ...
            "login_channel": "internal-shared-workspace",
            "is_superuser": True,
            "creator": "system",
            "status": "1",
            "is_active": "1",
    ...
    if not result.get("success"):
        raise RuntimeError("DOCMIND_SHARED_USER_CREATE_FAILED")
    logging.info("DocMind shared workspace identity initialized")
    return result["user_info"]["id"]
