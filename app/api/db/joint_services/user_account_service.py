...
def delete_user_data(user_id: str) -> dict:
    ...
    protected_scope_doc_ids = []
    if owned_tenant:
        owned_kb_ids = KnowledgebaseService.get_kb_ids(usr.id)
        if owned_kb_ids:
            protected_scope_doc_ids.extend(
                doc["id"] for doc in DocumentService.get_all_doc_ids_by_kb_ids(owned_kb_ids)
            )
    protected_scope_doc_ids.extend(
        doc["id"] for doc in DocumentService.get_all_docs_by_creator_id(usr.id)
    )
    try:
