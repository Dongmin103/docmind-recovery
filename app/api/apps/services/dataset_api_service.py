...
from api.db.services.document_service import DocumentService, DocmindProtectedEvidenceError, queue_raptor_o_graphrag_tasks
...
def delete_knowledge_graph(dataset_id: str, tenant_id: str):
    ...
    try:
        DocumentService.assert_documents_docmind_evidence_mutable(
            [document.id for document in DocumentService.query(kb_id=dataset_id)],
...
def run_index(dataset_id: str, tenant_id: str, index_type: str):
    ...
    try:
        DocumentService.assert_documents_docmind_evidence_mutable(
            ...
            f"RUN_{index_type.upper()}_INDEX",
            actor_id=tenant_id,
        )
    except DocmindProtectedEvidenceError as error:
        return False, error.code
...
def delete_index(dataset_id: str, tenant_id: str, index_type: str, wipe: bool = True):
    ...
    try:
        DocumentService.assert_documents_docmind_evidence_mutable(
            [document.id for document in DocumentService.query(kb_id=dataset_id)],
            f"DELETE_{index_type.upper()}_INDEX",
            actor_id=tenant_id,
        )
    except DocmindProtectedEvidenceError as error:
        return False, error.code
...
async def search(dataset_id: str, tenant_id: str, req: dict):
    ...
    rerank_candidate_limit = req.get("rerank_candidate_limit")
    if rerank_id:
        rerank_model_config = resolve_model_config(kb.tenant_id, LLMType.RERANK.value, rerank_id)
        rerank_mdl = LLMBundle(kb.tenant_id, rerank_model_config)
    elif rerank_candidate_limit is not None:
        return False, "rerank_candidate_limit requires rerank_id"
...
async def search_datasets(tenant_id: str, req: dict, *, candidate_mode: str = "hybrid"):
    """
    ...
    if isinstance(candidate_vector_similarity_weight, bool):
        return False, "candidate_vector_similarity_weight must be a finite number in [0, 1]"
    try:
