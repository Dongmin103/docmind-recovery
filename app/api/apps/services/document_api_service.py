...
def update_document_name_only(document_id, req_doc_name):
    ...
    """
    try:
        DocumentService.assert_docmind_evidence_mutable(document_id, "RENAME_DOCUMENT")
...
def update_chunk_method(req, doc, tenant_id):
    ...
    """
    try:
        DocumentService.assert_docmind_evidence_mutable(doc.id, "UPDATE_CHUNK_METHOD")
    except DocmindProtectedEvidenceError as error:
        return get_error_data_result(message=error.code)
...
def reset_document_for_reparse(doc, tenant_id, parser_id=None, pipeline_id=None):
    ...
    try:
        DocumentService.assert_docmind_evidence_mutable(doc.id, "RESET_DOCUMENT_FOR_REPARSE")
    except DocmindProtectedEvidenceError as error:
        return get_error_data_result(message=error.code)

    prospective = doc.to_dict()
    if parser_id is not None:
        prospective["parser_id"] = parser_id
    if pipeline_id is not None:
        prospective["pipeline_id"] = pipeline_id
    try:
        from rag.parser_platform.gates import enforce_prequeue_gate
        from rag.parser_platform.errors import ParserPlatformError

        preserve_active_chunks = enforce_prequeue_gate(prospective)
...
def update_document_status_only(status: int, doc, kb):
    ...
    """
    try:
        DocumentService.assert_docmind_evidence_mutable(doc.id, "UPDATE_DOCUMENT_AVAILABILITY")
    except DocmindProtectedEvidenceError as error:
        return get_error_data_result(message=error.code)
