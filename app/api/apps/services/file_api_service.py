...
async def delete_files(uid: str, file_ids: list, auth_header: str = ""):
    ...
    def _collect_document_ids(file):
        document_ids = [inform.document_id for inform in File2DocumentService.get_by_file_id(file.id)]
        if file.type == FileType.FOLDER.value:
            for child in FileService.list_all_files_by_parent_id(file.id):
                document_ids.extend(_collect_document_ids(child))
        return document_ids

    def _preflight_docmind_protection():
        document_ids = []
        for file_id in file_ids:
            exists, file = FileService.get_by_id(file_id)
            if exists and file and check_file_team_permission(file, uid):
                document_ids.extend(_collect_document_ids(file))
    ...
    try:
        await thread_pool_exec(_preflight_docmind_protection)
