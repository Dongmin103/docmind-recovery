...
async def metadata_batch_update(dataset_id, tenant_id):
    ...
    try:
        DocumentService.assert_documents_docmind_evidence_mutable(
            target_doc_ids,
            "UPDATE_DOCUMENT_METADATA_BATCH",
            actor_id=tenant_id,
        )
    except DocmindProtectedEvidenceError as error:
        return get_error_data_result(message=error.code)
...
def _run_sync(user_id: str, req):
    ...
    operation = (
        "STOP_PARSING_DOCUMENT_BATCH"
        if str(req["run"]) == TaskStatus.CANCEL.value
        else "PARSE_DOCUMENT_BATCH"
    )
    try:
        DocumentService.assert_documents_docmind_evidence_mutable(
            req["doc_ids"],
        ...
        return RetCode.DATA_ERROR, error.code
    ...
    for doc_id in req["doc_ids"]:
        ...
        preserve_active_chunks = False
        if str(req["run"]) == TaskStatus.RUNNING.value:
            from rag.parser_platform.gates import enforce_prequeue_gate

            preserve_active_chunks = enforce_prequeue_gate(doc.to_dict())
            if preserve_active_chunks:
                info.pop("chunk_num", None)
                info.pop("token_num", None)
        ...
        if all([rerun_with_delete, not preserve_active_chunks, str(doc.run) == TaskStatus.DONE.value]):
            DocumentService.clear_chunk_num_when_rerun(doc_id)

        DocumentService.update_by_id(doc_id, info)
        if req.get("delete", False) and not preserve_active_chunks:
    ...
    try:
        ...
        def _run_sync():
            ...
            for doc_id in valid_doc_ids:
                ...
                if str(doc.run) == TaskStatus.DONE.value and not preserve_active_chunks:
    ...
    try:
        ...
        def _run_sync():
            ...
            for doc_id in valid_doc_ids:
                ...
                preserve_active_chunks = preserve_active_evidence(doc, tasks=tasks)
                cancel_all_task_of(doc_id)
...
async def batch_update_document_status(tenant_id, dataset_id):
    ...
    owned_doc_ids = [doc.id for doc in DocumentService.query(kb_id=dataset_id) if doc.id in requested_ids]
    try:
        DocumentService.assert_documents_docmind_evidence_mutable(
            owned_doc_ids,
            "UPDATE_DOCUMENT_AVAILABILITY_BATCH",
            actor_id=tenant_id,
        )
    except DocmindProtectedEvidenceError as error:
        return get_error_data_result(message=error.code)
