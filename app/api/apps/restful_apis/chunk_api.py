...
from api.db.services.document_service import DocumentService, DocmindProtectedEvidenceError
...
from rag.utils.chunk_id import make_manual_chunk_id
...
def _docmind_mutation_denial(document_id, operation):
    try:
        DocumentService.assert_docmind_evidence_mutable(document_id, operation)
    except DocmindProtectedEvidenceError as error:
        return get_error_data_result(message=error.code)
    return None


def _docmind_batch_mutation_denial(document_ids, operation, actor_id):
    try:
...
def _parser_platform_chunk_fields(chunk):
    metadata = chunk.get("metadata") if isinstance(chunk.get("metadata"), dict) else {}
    parser_platform = metadata.get("parser_platform") if isinstance(metadata.get("parser_platform"), dict) else None
    if not parser_platform:
        return {}
    return {
        "parse_run_id": chunk.get("parse_run_id") or parser_platform.get("parse_run_id"),
        "chunk_set_id": chunk.get("chunk_set_id") or parser_platform.get("chunk_set_id"),
        "parser_platform": parser_platform,
        "provenance": parser_platform.get("contributing_provenance") or [],
        "office_locator": parser_platform.get("office_locator"),
        "hwp_locator": parser_platform.get("hwp_locator"),
        "display_html": parser_platform.get("display_html"),
    }


def _chunk_visible_for_document(chunk, document) -> bool:
    from api.db.services.active_chunk_scope_service import ActiveChunkScopeService

    return ActiveChunkScopeService.chunk_is_visible(chunk, document=document)
...
async def parse(tenant_id, dataset_id):
    ...
    if denial := _docmind_batch_mutation_denial(doc_list, "PARSE_DOCUMENT_BATCH", tenant_id):
        return denial
    ...
    for id in doc_list:
        ...
        preserve_active_chunks = enforce_prequeue_gate(doc[0].to_dict())
...
async def stop_parsing(tenant_id, dataset_id):
    ...
    unique_doc_ids, duplicate_messages = check_duplicate_ids(doc_list, "document")
    doc_list = unique_doc_ids
    if denial := _docmind_batch_mutation_denial(doc_list, "STOP_PARSING_DOCUMENT_BATCH", tenant_id):
        return denial
    ...
    for id in doc_list:
        ...
        from rag.parser_platform.gates import preserve_active_evidence

        preserve_active_chunks = preserve_active_evidence(doc[0], tasks=tasks)
...
async def list_chunks(tenant_id, dataset_id, document_id):
    ...
    if req.get("id"):
        ...
        if chunk.get("compile_kwd"):
            return get_result(message=f"Chunk not found: {dataset_id}/{req.get('id')}", code=RetCode.DATA_ERROR)
        if not _chunk_visible_for_document(chunk, doc):
            return get_result(message=f"Chunk not found: {dataset_id}/{req.get('id')}", code=RetCode.DATA_ERROR)
        ...
        final_chunk = {
            ...
            **_parser_platform_chunk_fields(chunk),
        }
        ...
        for chunk_id in sres.ids:
            d = {
                ...
                **_parser_platform_chunk_fields(sres.field[chunk_id]),
            }
            res["chunks"].append(d)
            _ = Chunk(**d)
...
async def get_chunk(tenant_id, dataset_id, document_id, chunk_id):
    ...
    try:
        ...
        if chunk.get("compile_kwd"):
            return get_result(data=False, message="Chunk not found!", code=RetCode.DATA_ERROR)
        if not _chunk_visible_for_document(chunk, doc[0]):
            return get_result(data=False, message="Chunk not found!", code=RetCode.DATA_ERROR)
        chunk.update(_parser_platform_chunk_fields(chunk))
...
async def delete_document_structure_graph(tenant_id, dataset_id, document_id):
    ...
    if denial := _docmind_mutation_denial(document_id, "DELETE_DOCUMENT_STRUCTURE_GRAPH"):
        return denial
    try:
        deleted = 0
...
async def add_chunk(tenant_id, dataset_id, document_id):
    ...
    if not doc:
        return get_error_data_result(message=f"you don't own the document {document_id}")
    doc = doc[0]
    if denial := _docmind_mutation_denial(document_id, "ADD_MANUAL_CHUNK"):
        return denial
    req = await get_request_json()
    ...
    explicit_chunk_id = req.get("id") or req.get("chunk_id")
    client_chunk_uid = req.get("client_chunk_uid") or req.get("source_chunk_uid")
    append_ordinal = req.get("manual_chunk_seq")
    needs_reserved_append_ordinal = append_ordinal is None and not client_chunk_uid and not explicit_chunk_id
    chunk_id = None
    if not needs_reserved_append_ordinal:
        chunk_id = make_manual_chunk_id(
            document_id,
            req["content"],
            explicit_id=explicit_chunk_id,
            client_chunk_uid=client_chunk_uid,
    ...
    if chunk_id:
        d["id"] = chunk_id
    if append_ordinal is not None:
        try:
            d["manual_chunk_seq_int"] = int(append_ordinal)
        except (TypeError, ValueError):
            d["manual_chunk_seq_kwd"] = str(append_ordinal)
    if client_chunk_uid:
        d["client_chunk_uid_kwd"] = str(client_chunk_uid)
    ...
    def store_index_and_count_chunk():
    ...
    if needs_reserved_append_ordinal:
        def add_reserved_chunk():
            nonlocal append_ordinal, chunk_id
            append_ordinal = DocumentService.next_manual_chunk_ordinal(doc.id, doc.kb_id)
            chunk_id = make_manual_chunk_id(document_id, req["content"], append_ordinal=append_ordinal)
            d["id"] = chunk_id
            d["manual_chunk_seq_int"] = append_ordinal
            return store_index_and_count_chunk()

        store_err = DocumentService.with_manual_chunk_append_lock(doc.id, add_reserved_chunk)
    else:
        store_err = store_index_and_count_chunk()
    if store_err:
...
async def rm_chunk(tenant_id, dataset_id, document_id):
    ...
    if not docs:
        return get_error_data_result(message=f"You don't own the document {document_id}.")
    if denial := _docmind_mutation_denial(document_id, "DELETE_MANUAL_CHUNK"):
        return denial
    req = await get_request_json()
    if not req:
        return get_result()
...
async def update_chunk(tenant_id, dataset_id, document_id, chunk_id):
    ...
    if not doc:
        return get_error_data_result(message=f"you don't own the document {document_id}")
    doc = doc[0]
    if denial := _docmind_mutation_denial(document_id, "UPDATE_MANUAL_CHUNK"):
        return denial
    chunk = settings.docStoreConn.get(chunk_id, search.index_name(dataset_tenant_id), [dataset_id])
    if chunk is None or str(chunk.get("doc_id", chunk.get("document_id"))) != str(document_id):
    ...
    try:
        ...
        def _switch_sync():
            ...
            if not doc or str(doc.kb_id) != str(dataset_id):
                return get_error_data_result(message="Document not found!")
            if denial := _docmind_mutation_denial(document_id, "SWITCH_MANUAL_CHUNKS"):
                return denial
