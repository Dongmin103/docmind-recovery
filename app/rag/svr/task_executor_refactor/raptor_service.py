...
class RaptorService:
    ...
    async def _generate_raptor(
        ...
        if not is_tree:
            ...
            for idx, (content, vctr, _, _) in enumerate(processed_chunks[original_length:], start=original_length):
                d = copy.deepcopy(doc)
                d["id"] = make_raptor_summary_chunk_id(content, doc_id, summary_ordinal=idx, layer_ordinal=chunk_layer.get(idx, 1))
        ...
        row_id = make_chunk_id("raptor_tree", doc_id, chunk_order_int="raptor")
        row = {
            **doc,
            "id": row_id,
    ...
    async def _generate_raptor_legacy_rows(
        ...
        for idx, item in enumerate(processed_chunks[original_length:], start=original_length):
            if len(item) >= 3:
                ...
                source_chunk_ids = []
            d = copy.deepcopy(doc)
            d["id"] = make_raptor_summary_chunk_id(content, doc_id, summary_ordinal=idx, layer_ordinal=chunk_layer.get(idx, 1), source_chunk_ids=source_chunk_ids)
            d["create_time"] = str(datetime.now()).replace("T", " ")[:19]
            d["create_timestamp_flt"] = datetime.now().timestamp()
            d[vctr_nm] = vctr.tolist()
            d["content_with_weight"] = content
    ...
    async def _persist_raptor_graph_to_es(self, doc_id: str) -> None:
        ...
        row_id = make_chunk_id("raptor_graph", doc_id, chunk_order_int=kb_id_str)
        row = {
            "id": row_id,
            "kb_id": kb_id_str,
            "doc_id": doc_id,
