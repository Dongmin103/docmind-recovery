...
class ChunkService:
    ...
    async def build_chunks(
        ...
        if ctx.parse_run_id:
            ...
            prepared, parser_run = ParserRunService.load_prepared_run(
                ...
                document={"id": ctx.doc_id, "name": ctx.name},
            ...
            def parser_progress(phase: str, details: dict) -> None:
                ...
                if lifecycle:
                    ParserRunService.update_lifecycle(ctx.parse_run_id, lifecycle)
                if "expected_pages" in details:
                    ParserRunService.update_page_progress(
                        ctx.parse_run_id,
                        expected_pages=int(details.get("expected_pages") or 0),
                        completed_pages=int(details.get("completed_pages") or 0),
                        reused_pages=int(details.get("reused_pages") or 0),
                        failed_pages=int(details.get("failed_pages") or 0),
                    )
                ctx.progress_cb(msg=f"Parser {phase}: {details}")

            try:
                parser_platform_document = await asyncio.to_thread(
                    ParserPlatformStandardBridge.from_config(config, progress=parser_progress).parse,
                    prepared=prepared,
                    source_bytes=parser_source,
                    source_document_id=ctx.doc_id,
                    source_format=SourceFormat(parser_run.source_format),
                    expected_page_count=parser_run.expected_page_count,
                    trace_id=ctx.id,
                )
            ...
            ctx._parser_platform_document = parser_platform_document
            ctx.recording_context.record(
                "parser_platform",
                {
                    "parse_run_id": ctx.parse_run_id,
                    "chunk_set_id": ctx.chunk_set_id,
                    "source_format": parser_platform_document.source_format.value,
                    "parser_name": parser_platform_document.parser_name,
                    "parser_version": parser_platform_document.parser_version,
                    "status": parser_platform_document.status.value,
                    "canonical_hash": parser_platform_document.canonical_hash,
                },
            )
        else:
            # Get parser
        ...
        # ParsedDocument already owns hierarchy; the legacy outline extractor
        # would flatten or duplicate it.
        if not ctx.parse_run_id:
        ...
        if ctx.parse_run_id:
            from rag.parser_platform import StagingChunkTagger

            docs = StagingChunkTagger.tag(
                docs,
    ...
    async def _prepare_docs_and_upload(self, cks: List[Dict]) -> List[Dict]:
        ...
        @timeout(60)
        async def upload_to_minio(document, chunk, chunk_order):
            try:
        ...
        for chunk_order, ck in enumerate(cks):
            tasks.append(asyncio.create_task(upload_to_minio(doc, ck, chunk_order)))
        try:
    ...
    async def insert_chunks(
        ...
        if self._task_context.parse_run_id:
            from rag.parser_platform import StagingChunkTagger

            chunks = StagingChunkTagger.tag(
                chunks,
