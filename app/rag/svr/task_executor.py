...
async def run_dataflow(task: dict):
    ...
    for chunk_order, ck in enumerate(chunks):
        ...
        ck["id"] = make_chunk_id("dataflow_chunk", ck["doc_id"], chunk=ck, chunk_order_int=chunk_order)
        if "questions" in ck:
...
async def delete_raptor_chunks(doc_id: str, tenant_id: str, kb_id: str, keep_method: str | None = None):
    ...
    DocumentService.assert_docmind_evidence_mutable(doc_id, "DELETE_RAPTOR_CHUNKS")
...
async def run_raptor_for_kb(row, kb_parser_config, chat_mdl, embd_mdl, vector_size, callback=None, doc_ids=[]):
    ...
    async def generate(chunks, did):
        ...
        for idx, (content, vctr) in enumerate(chunks[original_length:], start=original_length):
            d = copy.deepcopy(doc)
            d["id"] = make_raptor_summary_chunk_id(content, did, summary_ordinal=idx, layer_ordinal=chunk_layer.get(idx, 1))
            d["create_time"] = str(datetime.now()).replace("T", " ")[:19]
            d["create_timestamp_flt"] = datetime.now().timestamp()
...
async def do_handle_task(task):
    ...
    if task_doc_id and task_doc_id not in {CANVAS_DEBUG_DOC_ID, GRAPH_RAPTOR_FAKE_DOC_ID}:
        DocumentService.assert_docmind_evidence_mutable(task_doc_id, f"TASK_EXECUTOR_{task_type.upper() or 'PARSE'}")
...
async def main():
    ...
    # Consumer indexes are identifiers, not reliable worker ordinals. Launchers
    # pass the ordinal explicitly so an index ending in a timestamp cannot delay
    # ingestion for years.
    if args.startup_delay_slot is not None:
        startup_delay = args.startup_delay_slot * 2.0 + random.uniform(0, 0.5)
        if startup_delay > 0:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--startup-delay-slot", type=int, default=None, help="Worker ordinal used only to stagger startup")
