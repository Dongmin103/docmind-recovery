...
NON_SPATIAL_COLUMN_ID = "__title_chunker_non_spatial_stream__"
...
class BaseTitleChunker(ABC):
    ...
    def extract_line_records(self):
        ...
        if payload is not None:
            ...
            return [
                {"text": line, "doc_type_kwd": "text", "img_id": None, "layout": "", "column_id": NON_SPATIAL_COLUMN_ID, PDF_POSITIONS_KEY: []}
        ...
        for item in items or []:
            ...
            if not positions and column_id is None:
                column_id = NON_SPATIAL_COLUMN_ID
            record = {
                ...
                "column_id": column_id,
                "reading_order_int": item.get("reading_order_int"),
                PDF_POSITIONS_KEY: positions,
            }
    ...
    def build_chunks_from_record_groups(self, record_groups):
        ...
        if self.from_upstream.output_format in ["markdown", "text", "html"]:
            chunks = [self._build_plain_text_chunk(records) for records in record_groups if records]
        else:
            chunks = [(self._build_text_chunk(records) if records[0]["doc_type_kwd"] == "text" else self._build_media_chunk(records[0])) for records in record_groups if records]
    ...
    @staticmethod
    def _is_non_searchable_text_group(records):
        return all(record.get("available_int") == 0 for record in records)

    @classmethod
    def _build_plain_text_chunk(cls, records):
        chunk = {"text": "".join(record["text"] + "\n" for record in records)}
        if cls._is_non_searchable_text_group(records):
            chunk["available_int"] = 0
        return chunk

    @staticmethod
    def _shared_column_id(records):
        column_ids = {record.get("column_id") for record in records}
        if len(column_ids) == 1:
            column_id = column_ids.pop()
            if column_id != NON_SPATIAL_COLUMN_ID:
                return column_id
        return None

    @classmethod
    def _build_text_chunk(cls, records):
        chunk = {
            "text": RAGFlowPdfParser.remove_tag("".join(record["text"] + "\n" for record in records)),
            "doc_type_kwd": "text",
            PDF_POSITIONS_KEY: merge_pdf_positions(records),
        }
        column_id = cls._shared_column_id(records)
        if column_id is not None:
            chunk["column_id"] = column_id
        if cls._is_non_searchable_text_group(records):
            chunk["available_int"] = 0
        return chunk

    @staticmethod
    def _build_media_chunk(record):
        chunk = {
            "text": record["text"],
            "doc_type_kwd": record["doc_type_kwd"],
            "img_id": record.get("img_id"),
            PDF_POSITIONS_KEY: record[PDF_POSITIONS_KEY],
        }
        for key in MEDIA_METADATA_KEYS:
            if key in record:
                chunk[key] = record[key]
        if record.get("available_int") == 0:
            chunk["available_int"] = 0
        return chunk
