...
def reorder_multi_column_bboxes(pdf_parser, bboxes, zoom=PDF_MULTI_COLUMN_ZOOM):
    """Order a detected multi-column PDF and persist its column boundary.

    DeepDOC has already assigned ``col_id`` while reconstructing layout.  The
    previous opt-in path used that information only to rearrange a transient
    list, allowing downstream chunkers to join adjacent left/right records
    again.  Give every emitted box a page-scoped column identity so the
    chunker can keep the physical columns separate.
    """
    ...
    ordered = pdf_parser.sort_X_by_page(bboxes, column_width / 2)
    for reading_order_int, box in enumerate(ordered):
        page_number = box.get("page_number")
        column = box.get("col_id")
        if page_number is not None and column is not None:
            box["column_id"] = f"{int(page_number)}:{int(column)}"
        box["reading_order_int"] = reading_order_int
    return ordered
...
async def restore_pdf_text_previews(chunks, from_upstream, canvas):
    ...
    try:
        ...
        logger.warning(f"Failed to load PDF page images for chunk preview restore: {e}")
        return
