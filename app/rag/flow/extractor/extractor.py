...
class Extractor(ProcessBase, LLM):
    ...
    async def _knowledge_compile(self, docs):
        ...
        self.callback(0.2, message="Start to compile document structure ...")
        docs = sorted(
            docs,
            key=lambda d: (
                d.get("page_num_int", 0)[0] if isinstance(d.get("page_num_int", 0), list) else d.get("page_num_int", 0),
        ...
        return await merge_compiled_structures(
            docs,
            self.chat_mdl,
            embedding_model,
            self._canvas.get_tenant_id(),
        ...
        if chunks:
            if self._param.field_name == "toc":
                for chunk_order, ck in enumerate(chunks):
                    ...
                    ck["id"] = make_chunk_id("extractor_toc_source", ck["doc_id"], chunk=ck, chunk_order_int=chunk_order)
            ...
            if self._param.field_name in ["set", "list", "graph"]:
                for chunk_order, ck in enumerate(chunks):
                    ck["doc_id"] = self._canvas._doc_id
                    ck["id"] = make_chunk_id("extractor_knowledge_source", ck["doc_id"], chunk=ck, chunk_order_int=chunk_order)
