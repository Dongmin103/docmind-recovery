...
def translate_active_chunk_filter(expression, *, safe_maximum: int = 10_000) -> dict:
    from rag.parser_platform.active_scope import InfinityActiveChunkFilterTranslator

    return InfinityActiveChunkFilterTranslator(safe_maximum=safe_maximum).translate(expression)
...
class InfinityConnection(InfinityConnectionBase):
    ...
    def search(
        ...
        active_filter = (
            translate_active_chunk_filter(coerce_active_chunk_filter(active_scope))["rendered_where"]
        ...
        try:
            ...
            if active_filter:
                filter_cond = f"({filter_cond}) AND ({active_filter})" if filter_cond else active_filter
