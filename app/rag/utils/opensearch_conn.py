...
def translate_active_chunk_filter(expression, *, safe_maximum: int = 10_000) -> dict:
    from rag.parser_platform.active_scope import OpenSearchActiveChunkFilterTranslator

    return OpenSearchActiveChunkFilterTranslator(safe_maximum=safe_maximum).translate(expression)
...
class OSConnection(DocStoreConnection):
    ...
    def search(
        ...
        if active_scope is not None:
            bqry.filter.append(Q(translate_active_chunk_filter(coerce_active_chunk_filter(active_scope))))
