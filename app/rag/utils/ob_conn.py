...
column_definitions: list[Column] = [
    ...
    Column("parse_run_id", String(64), nullable=True, index=True, comment="parser-platform run id"),
    Column("chunk_set_id", String(64), nullable=True, index=True, comment="active/staging chunk set id"),
...
def translate_active_chunk_filter(expression, *, safe_maximum: int = 10_000) -> dict:
    from rag.parser_platform.active_scope import OceanBaseActiveChunkFilterTranslator

    return OceanBaseActiveChunkFilterTranslator(safe_maximum=safe_maximum).translate(expression)
    ...
    def search(
        ...
        **kwargs,
        ...
        from rag.parser_platform.active_scope import (
            ACTIVE_CHUNK_SCOPE_CONDITION,
            ElasticsearchActiveChunkFilterTranslator,
            coerce_active_chunk_filter,
        )

        active_scope = condition.pop(ACTIVE_CHUNK_SCOPE_CONDITION, None)
        active_expression = coerce_active_chunk_filter(active_scope) if active_scope is not None else None
        ...
        if len(match_expressions) == 3 and self.es:
            ...
            if active_expression is not None:
                bqry.filter.append(Q(ElasticsearchActiveChunkFilterTranslator().translate(active_expression)))
        ...
        if active_expression is not None:
            active_filter = translate_active_chunk_filter(active_expression)["rendered_where"]
            filters_expr = f"({filters_expr}) AND ({active_filter})" if filters_expr else active_filter
