...
def translate_active_chunk_filter(expression, *, safe_maximum: int = 10_000) -> dict:
    from rag.parser_platform.active_scope import ElasticsearchActiveChunkFilterTranslator

    return ElasticsearchActiveChunkFilterTranslator(safe_maximum=safe_maximum).translate(expression)


def normalize_search_result_field(name: str, value):
...
class ESConnection(ESConnectionBase):
    ...
    def search(
        ...
        from rag.parser_platform.active_scope import ACTIVE_CHUNK_SCOPE_CONDITION, coerce_active_chunk_filter

        active_scope = condition.pop(ACTIVE_CHUNK_SCOPE_CONDITION, None)

        bool_query = Q("bool", must=[])
        if active_scope is not None:
            bool_query.filter.append(Q(translate_active_chunk_filter(coerce_active_chunk_filter(active_scope))))
        ...
        for hit in hits:
            ...
            for n in fields:
                ...
                # Then check fields (ES 9.x stores dense_vector here, not in _source)
                    ...
                    m[n] = vals
            for n, v in m.items():
                m[n] = normalize_search_result_field(n, v)
                # if n.find("tks") > 0:
