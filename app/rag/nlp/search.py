...
class Dealer:
    ...
    async def search(self, req, idx_names: str | list[str], kb_ids: list[str], emb_mdl=None, highlight: bool | list | None = None, rank_feature: dict | None = None, min_match: bool = True):
        ...
        filters = await asyncio.to_thread(
            ActiveChunkScopeService.apply_to_condition,
            filters,
            kb_ids=kb_ids,
        ...
        try:
            candidate_vector_similarity_weight = float(candidate_vector_similarity_weight)
        except (TypeError, ValueError) as error:
            raise ValueError("candidate_vector_similarity_weight must be a finite number in [0, 1]") from error
        if not math.isfinite(candidate_vector_similarity_weight) or not 0 <= candidate_vector_similarity_weight <= 1:
            raise ValueError("candidate_vector_similarity_weight must be a finite number in [0, 1]")
        q_vec = []
        if not qst:
            if req.get("sort"):
            ...
            if emb_mdl is None:
                ...
                if candidate_mode == "dense":
                    ...
                    logging.debug(
                        "Dealer.search fusion: knn_top_k=%s candidate_vector_similarity_weight=%s",
                        knn_top_k,
                        candidate_vector_similarity_weight,
                    )
                    fusionExpr = build_fusion_expr(knn_top_k, candidate_vector_similarity_weight)
                    ...
                    fusionExpr = FusionExpr(
                        "weighted_sum",
                        knn_top_k,
                        {"weights": f"{1 - candidate_vector_similarity_weight:g},{candidate_vector_similarity_weight:g}"},
                    )
                    matchExprs = [matchText, matchDense, fusionExpr] if matchText else [matchDense]
                else:
                    fusionExpr = FusionExpr(
                ...
                if total == 0 and candidate_mode == "hybrid":
    ...
    @staticmethod
    def _rerank_window(page_size: int, top: int = 0, candidate_limit: int | None = None) -> int:
        ...
        if candidate_limit is not None:
            if candidate_limit < page_size:
                raise ValueError("rerank_candidate_limit must be at least page_size")
            if top > 0 and candidate_limit > top:
                raise ValueError("rerank_candidate_limit must not exceed top_k")
            if candidate_limit % page_size != 0:
                raise ValueError("rerank_candidate_limit must be a multiple of page_size")
    ...
    async def retrieval(
        ...
        *,
        ...
        candidate_vector_similarity_weight: float = 0.95,
    ):
        """
        ...
        page = max(page, 1)
        candidate_window = rerank_candidate_limit if rerank_candidate_limit is not None else rerank_candidates_count
        if candidate_window < page_size:
            raise ValueError("rerank candidate window must be at least page_size")
        if page * page_size > candidate_window:
            raise ValueError(
                f"rerank candidate window({candidate_window}) must be greater than or equal to page({page}) * page_size({page_size})"
            )
        ...
        req = {
            ...
            "candidate_vector_similarity_weight": candidate_vector_similarity_weight,
        }
        if isinstance(must_not, dict) and must_not:
            req["must_not"] = must_not
        logging.debug(f"[Search] page={page}, page_size={page_size}, candidate_window={candidate_window}")
    ...
    def chunk_list(
        ...
        condition = ActiveChunkScopeService.apply_to_condition(
            {"doc_id": doc_id},
            kb_ids=kb_ids,
