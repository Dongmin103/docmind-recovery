...
def _redact_config_value(value, key=None):
    normalized_key = str(key or "").strip().lower()
    if normalized_key in _SENSITIVE_CONFIG_KEYS or normalized_key.endswith(_SENSITIVE_CONFIG_KEY_SUFFIXES):
        return "*" * 8
    if isinstance(value, dict):
        return {nested_key: _redact_config_value(nested_value, nested_key) for nested_key, nested_value in value.items()}
